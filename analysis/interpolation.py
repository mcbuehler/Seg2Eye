...
def run_interpolation(index_start, index_end, n=5, write=False, show=False):
    ...
    original_start = ImagePostprocessor.to_255resized_imagebatch(style_image_to_grid(data_start), as_tensor=False)[0][0].astype(np.uint8)
    original_end = ImagePostprocessor.to_255resized_imagebatch(style_image_to_grid(data_end), as_tensor=False)[0][0].astype(
        np.uint8)

    # results.append(original_start)
...
# stack(combos)
for combo in combos:
    interpolation_video(combo[0], combo[1], n=100)
